...
// Note: ResourceDescriptor does NOT carry an id field — the registry assigns
// ResourceIds on registration and returns them. Callers must not pre-generate IDs.
...
impl ResourceRegistry {
    pub fn register(&mut self, desc: ResourceDescriptor) -> ResourceId  // registry generates and owns the ID
...
impl KvasirGraph {
    ...
    pub fn plan(&self) -> Result<ExecutionPlan, KvasirError>  // renamed from compile() to avoid collision with MaterialCompiler::compile()
}
```
...
impl ExecutionPlanner {
    pub fn plan(graph: &KvasirGraph, registry: &ResourceRegistry) -> Result<ExecutionPlan, KvasirError> {
        // 1. Topological sort (Kahn's algorithm — cycle here is a validate() contract
        //    violation; validate() must be called before plan(). Returns Err, not panic.
...
pub struct ExecutionPlan {
    ...
    // Transient aliasing: two ResourceIds whose read/write lifetimes do not overlap
    // can be mapped to the same underlying GPU allocation. The registry reuses that
    // memory instead of allocating a new texture, reducing VRAM pressure for
    // intermediate resources that are produced and consumed within the same frame.
    pub resource_aliases: HashMap<ResourceId, ResourceId>,
}
```
...
pub struct GlassNode {
    ...
    pub label:         &'static str, // debug identity — used when multiple GlassNodes exist in the same graph
}
...
```rust
// After: graph-driven
let plan = self.kvasir_graph.plan()?;
...
fn build_frame_graph(
    ...
    // scene_color uses FrameContent: the GPU texture allocation persists across frames
    // (no per-frame realloc cost), but the pixel content is rebuilt from scratch each frame.
    // Use Frame (fully transient) only for true scratch textures discarded within one effect.
    ...
    let bloom_a = registry.get_or_create_image(&ResourceDescriptor {
        ...
        lifetime: ResourceLifetime::FrameContent,  // allocation persists, content rebuilt each frame
        ..Default::default()
    });
...
fn build_multi_effect_graph(
    ...
    // All three consumers depend on the pyramid.
    // However, modal_glass and tooltip_glass both write to scene_color, so their
    // ordering matters for correct z-layering: modal composites first (it sits behind
    // the tooltip), then tooltip composites on top. This must be explicit — the planner
    // cannot infer draw order from resource identity alone when two nodes write the
    // same resource.
    graph.connect(pyramid_node, modal_blur_resource,   modal_glass);
    graph.connect(modal_glass,  scene_color,           tooltip_glass);  // enforce z-order
    graph.connect(pyramid_node, tooltip_blur_resource, tooltip_glass);
    graph.connect(pyramid_node, pyramid.luminance,     focus_node);
    // focus_node reads luminance only — it does not write scene_color, so it can run
    // in parallel with the glass nodes if the GPU scheduler allows it.
...
pub struct LayerCompositeNode  { pub blend_mode: BlendMode, pub opacity: f32 }  // renamed to avoid collision with render-graph CompositeNode
```
...
let turbulence = mat.add(LayerCompositeNode {
...
let shimmer = mat.add(LayerCompositeNode {
