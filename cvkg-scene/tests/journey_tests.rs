use cvkg_core::{Rect, View, FrameRenderer, Event};
use cvkg_scene::test_renderer::{TestRenderer, Command};
use cvkg_vdom::VDom;
use cvkg_components::{Button, VStack, Text};
use std::sync::Arc;
...
fn journey_button_click_flow() {
    ...
    assert!(vdom.nodes.values().any(|n| n.component_type.contains("Button")));
    
    // 3. INTERACTION: Find the button node and dispatch a click event
    let button_node = vdom.nodes.values()
    ...
    assert!(clicked.load(Ordering::SeqCst), "Button click handler was not triggered");
}

#[test]
fn journey_layout_reflow_on_content_change() {
    // 1. SETUP: Initial view with short text
    let rect = Rect { x: 0.0, y: 0.0, width: 500.0, height: 500.0 };
    let view1 = Text::new("Short");
    let vdom1 = VDom::build(&view1, rect);
    
    let text_node1 = vdom1.nodes.values()
    ...
    let text_node2 = vdom2.nodes.values()
        .find(|n| n.component_type.contains("Text"))
        .expect("Text node not found");
    let updated_width = text_node2.layout.width;
    
    assert!(updated_width > initial_width, "Layout did not reflow after content change");
    
    // 3. DIFF: Verify that VDOM patch is generated
    let patches = vdom1.diff(&vdom2);
    assert!(!patches.is_empty(), "No patches generated for content change");
}
...
fn journey_complex_hierarchy_rendering() {
    ...
    let rect = Rect { x: 0.0, y: 0.0, width: 300.0, height: 600.0 };
    let mut renderer = TestRenderer::new();
    view.render(&mut renderer, rect);
    ...
    assert!(commands.iter().any(|c| matches!(c, Command::PushVNode { name, .. } if name.contains("VStack"))));
    assert!(commands.iter().any(|c| matches!(c, Command::PushVNode { name, .. } if name.contains("Button"))));
    assert!(commands.iter().any(|c| matches!(c, Command::FillRect { .. })));
    assert!(commands.iter().filter(|c| matches!(c, Command::DrawText { .. })).count() >= 3);
}
