...
// Uses a relative luminance gate to isolate high-energy light sources.
// For volumetric effects, we need a lower threshold to captured the cyan glow.
@fragment
fn fs_bloom_extract(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Lower threshold to capture volumetric glow
...
fn fs_composite(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ACES Filmic Tonemapping to map HDR to LDR
