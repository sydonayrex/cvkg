...
mod stubs {
    use cvkg_core::{ElapsedTime, FrameRenderer, Rect, RenderTier, Renderer};
    ...
    impl WebRenderer {
        ...
        pub async fn forge(&mut self) -> Result<RenderTier, String> {
            Ok(RenderTier::Tier3Fallback)
        }
        pub fn canvas(&self) -> Option<&()> {
            None
        }
        pub fn update_vdom<V>(&mut self, _: V) -> Result<(), String> {
            Ok(())
        }
    ...
    impl Renderer for WebRenderer {
        ...
        fn measure_text(&mut self, _: &str, _: f32) -> (f32, f32) {
            (0.0, 0.0)
        }
    ...
    mod tests {
        ...
        fn test_compute_struct_layouts() {
            use crate::{ComputeParams, SceneNode};
...
mod wasm_impl {
    ...
    use super::{ComputeParams, SceneNode};
    use cvkg_core::{AssetManager, ElapsedTime, FrameRenderer, Rect, RenderTier, Renderer, View};
    ...
    static CURRENT_VDOM: std::sync::OnceLock<
        std::sync::Mutex<Option<std::sync::Arc<cvkg_vdom::VDom>>>,
                ...
                match self.init_webgpu_async().await {
                    ...
                    Err(e) => {
                        ...
                        match self.init_webgl2() {
                            ...
                            Err(e2) => {
                                log::warn!(
                                    "WebGL2 initialization failed: {:?}. Using Tier 3 Canvas2D.",
                                    e2
                                );
        ...
        async fn init_webgpu_async(&mut self) -> Result<(), JsValue> {
            ...
            if gpu.is_undefined() {
                ...
                return Err(JsValue::from_str(
                    "WebGPU not supported by browser (navigator.gpu undefined)",
                ));
            }
    ...
    impl Renderer for WebRenderer {
        ...
        fn bifrost(&mut self, rect: Rect, blur: f32, _saturation: f32, opacity: f32) {
            if let Some(ref gl) = self.canvas_context {
                ...
                let has_filter =
                    js_sys::Reflect::has(gl, &wasm_bindgen::JsValue::from_str("filter"))
                        .unwrap_or(false);
    ...
    impl WebRenderer {
        ...
        fn draw_background(&self, ctx: &web_sys::CanvasRenderingContext2d, time: f32) {
            ...
            for i in 0..3 {
                ...
                ctx.set_stroke_style(&wasm_bindgen::JsValue::from_str(&format!(
                    "rgba(0, 200, 255, {})",
                    alpha * 2.0
                )));
        ...
        fn register_web_events(&self) -> Result<(), JsValue> {
            ...
            on_pointer_event("pointerdown", |x, y| cvkg_core::Event::PointerDown {
                x,
                y,
                button: 0,
            })?;
        ...
        fn load_image(&self, url: &str) -> cvkg_core::AssetState<std::sync::Arc<Vec<u8>>> {
            ...
            // Mark as Loading synchronously via atomic rcu
            {
                ...
                self.cache.rcu(move |map| {
                    let mut m = (**map).clone();
                    m.entry(key.clone())
                        .or_insert(cvkg_core::AssetState::Loading);
                    m
                });
            }
