...
## errors in the document that need further clarification or fixing:

Errors
Line 204 — KvasirGraph::compile() name collision with MaterialCompiler::compile(). KvasirGraph has pub fn compile(&self) -> Result<ExecutionPlan, KvasirError> and MaterialCompiler has pub fn compile(graph: &MaterialGraph) -> Result<CompiledMaterial, CompileError>. Both methods are named compile but return completely different types. This is legal Rust since they're on different types, but it becomes a real problem at line 289 where self.kvasir_graph.compile()? returns an ExecutionPlan, while in the material examples MaterialCompiler::compile(&mat)? returns a CompiledMaterial. The ambiguity will confuse readers and likely confuse the weak AI. KvasirGraph::compile() should be renamed to KvasirGraph::plan() to match the ExecutionPlanner terminology already used throughout the document — it produces a plan, not a compilation artifact.
Line 217 — The planner comment says "panic on cycle" but should return an error. The comment reads // 1. Topological sort (Kahn's algorithm — panic on cycle). A panic is the wrong behavior for a cycle in user-submitted or AI-submitted graph data. The validator (graph.validate()) at line 203 is responsible for catching cycles before plan() is called — if a cycle reaches the planner it means validate() was skipped, which is a programmer error and a panic is arguably defensible there. But the comment should say // 1. Topological sort (Kahn's algorithm — cycle is a validate() contract violation, returns Err) to make the expected calling convention explicit and avoid someone reading this as intentional panic behavior.
Line 561 — CompositeNode appears in both the render graph node list (line 262) and the material node list (line 561) with the same name and different fields. In Phase 2, CompositeNode is a KvasirNode with scene_input, bloom_input, and output as ResourceId fields — it composites the final frame. In Phase 4, CompositeNode is a MaterialNode with blend_mode: BlendMode and no ResourceId fields — it blends two material layer inputs. Same naming collision problem as GlassNode was, fixed to BifrostMaterialNode, but CompositeNode was missed. The material-layer version should be BlendNode or LayerCompositeNode.
Line 667 — The right-way example reverts to mat.compile()? instead of MaterialCompiler::compile(&mat)?. The previous fix standardized on MaterialCompiler::compile(&mat)? throughout, but the right-way example at line 667 still calls mat.compile()? directly on the graph. This is the exact inconsistency the previous round fixed in the simple example. It needs to match.
Line 752 — impl KvasirGenerator for AiMaterialGenerator still declares fn generate(&self) -> Result<KvasirGraph, GeneratorError> instead of Result<(), GeneratorError>. The trait definition at line 726 was correctly updated to Result<(), GeneratorError>, but the impl block signature on line 752 still says Result<KvasirGraph, GeneratorError>. The trait and its implementation disagree, which would be a compile error. The impl signature needs to match the trait.
Lines 480–486 — GlassNode struct literal includes a label field that doesn't exist in the GlassNode definition at line 245. The GlassNode struct has blur_input, scene_input, and scene_output. The multi-consumer pyramid example at line 482 adds label: "modal_glass" and line 495 adds label: "tooltip_glass". No label field exists on the struct. Either add pub label: &'static str to GlassNode's definition in section 2.4, or remove it from the example and use the node's position in the graph as its identity.

Points of Clarification
Line 100–101 — ResourceDescriptor has an id field but register() also returns a ResourceId. The struct contains pub id: ResourceId and register(&mut self, desc: ResourceDescriptor) -> ResourceId. This implies the caller must supply an ID before registration and the registry also hands one back — two IDs for one resource. The pattern should be one or the other. Either the caller generates the ID and the registry stores it (the id field stays, return type becomes ()), or the registry generates the ID and the caller provides no ID (remove id from the descriptor, keep the ResourceId return). The get_or_create_image call pattern used throughout the examples suggests the registry assigns IDs, so the id field on ResourceDescriptor should be removed.
Line 217 — "Transient resource aliasing" in the execution plan is not explained anywhere. resource_aliases: HashMap<ResourceId, ResourceId> appears in ExecutionPlan with a comment "transient aliasing" but the concept is never defined. Transient aliasing means two ResourceIds that don't overlap in their read/write lifetime can be mapped to the same GPU allocation — the registry reuses the memory. This is an important optimization (avoids allocating a new texture every frame for intermediate results) but a reader encountering resource_aliases for the first time has no context for what it does or how the execution engine uses it. A one-sentence clarifying comment in the planner's step list would close this gap.
Lines 482–496 — Two GlassNode instances both write to scene_color with no ordering between them. modal_glass and tooltip_glass both have scene_output: scene_color. The text says "the planner can schedule them in any order after pyramid_node completes" — but that's only safe if one glass node's output doesn't need to be read by the other. Since both overdraw onto scene_color, the order does matter: modal should composite first (it's behind the tooltip), then tooltip composites on top. If the planner schedules them in arbitrary order, the layering will be wrong on frames where the planner chooses tooltip first. The example needs a graph.connect(modal_glass, scene_color, tooltip_glass) to enforce the z-order, and the accompanying explanation should note that same-resource write ordering must always be explicit.
Line 310 — scene_color uses ResourceLifetime::Frame but the note about FrameContent added in Phase 1 makes Frame ambiguous. With the new FrameContent variant, Frame now means "allocation AND content are transient — freed at end of frame." But scene_color is not truly freed every frame — the GPU texture backing it persists; only the rendered pixels are new each frame. By the FrameContent definition introduced in section 1.1, scene_color should also use FrameContent, not Frame. The distinction in the example contradicts the definitions provided. Frame should be reserved for genuinely ephemeral scratch allocations that are truly discarded (e.g. a temporary intermediate texture used within a single effect and never referenced again).
