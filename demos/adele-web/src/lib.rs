...
impl AdeleApp {
    fn render_header(&self, renderer: &mut dyn Renderer, rect: Rect) {
        ...
        renderer.draw_line(rect.x, rect.y + rect.height, rect.x + rect.width, rect.y + rect.height, [0.0, 0.8, 1.0, 0.3], 1.5);
        
        let title = "ADELE";
        renderer.draw_text(title, rect.x + 30.0, rect.y + 25.0, 28.0, [0.0, 1.0, 1.0, 1.0]);
        renderer.draw_text("DESIGN SYSTEMS EXPLORER", rect.x + 120.0, rect.y + 32.0, 12.0, [0.5, 0.5, 0.6, 1.0]);
        
        // Navigation
        let mut x = rect.x + 350.0;
        let navs = [("Catalog", ViewMode::Catalog), ("Comparison", ViewMode::Comparison)];
        for (label, mode) in navs {
            let is_active = self.view_mode == mode;
            renderer.draw_text(label, x, rect.y + 30.0, 14.0, if is_active { [1.0, 1.0, 1.0, 1.0] } else { [0.5, 0.5, 0.5, 1.0] });
            if is_active {
                renderer.draw_line(x, rect.y + 50.0, x + 50.0, rect.y + 50.0, [0.0, 0.8, 1.0, 1.0], 2.0);
            }
            x += 100.0;
        }

        let stats = format!("{} SYSTEMS", self.systems.iter().filter(|s| self.criteria.matches(s)).count());
        let (sw, _) = renderer.measure_text(&stats, 12.0);
        renderer.draw_text(&stats, rect.x + rect.width - sw - 30.0, rect.y + 30.0, 12.0, [0.0, 0.8, 1.0, 1.0]);
    }

    fn render_sidebar(&self, renderer: &mut dyn Renderer, rect: Rect) {
        renderer.fill_rect(rect, [0.02, 0.02, 0.05, 0.7]);
        renderer.draw_line(rect.x + rect.width, rect.y, rect.x + rect.width, rect.y + rect.height, [0.2, 0.2, 0.3, 0.5], 1.0);
        
        renderer.draw_text("FILTERS", rect.x + 25.0, rect.y + 30.0, 10.0, [0.4, 0.4, 0.5, 1.0]);
        
        // Search Input
        let search_rect = Rect { x: rect.x + 15.0, y: rect.y + 60.0, width: rect.width - 30.0, height: 40.0 };
        ...
        renderer.draw_text(if self.criteria.search_query.is_empty() { "Search systems..." } else { &self.criteria.search_query }, search_rect.x + 12.0, search_rect.y + 12.0, 14.0, [0.5, 0.5, 0.6, 1.0]);
        
        // Filter Categories
        let mut y = rect.y + 130.0;
        let categories = [
            ("TECHNOLOGY", vec!["React", "Vue", "TypeScript", "Web Components"]),
    ...
    fn render_grid(&self, renderer: &mut dyn Renderer, rect: Rect) {
        let filtered: Vec<&DesignSystem> = self.systems.iter()
    ...
    fn render_system_card(&self, renderer: &mut dyn Renderer, rect: Rect, system: &DesignSystem) {
        ...
        // Header
        renderer.draw_text(&system.system.data.to_uppercase(), rect.x + 20.0, rect.y + 20.0, 18.0, [1.0, 1.0, 1.0, 1.0]);
        renderer.draw_text(&system.company.data, rect.x + 20.0, rect.y + 45.0, 14.0, [0.0, 0.8, 1.0, 1.0]);
        ...
        if system.ts.data.to_lowercase() == "yes" {
            ...
            ts_badge.render(renderer, Rect { x: badge_x, y: badge_y, width: 35.0, height: 22.0 });
            badge_x += 42.0;
        }
        ...
        if js.contains("react") {
            ...
            react_badge.render(renderer, Rect { x: badge_x, y: badge_y, width: 55.0, height: 22.0 });
            badge_x += 62.0;
        } else if js.contains("vue") {
            let vue_badge = Badge::new("Vue").variant(BadgeVariant::Outline);
            vue_badge.render(renderer, Rect { x: badge_x, y: badge_y, width: 45.0, height: 22.0 });
            badge_x += 52.0;
        }
        
        if system.accessibility_guidelines.data.to_lowercase() == "yes" {
             let a11y_badge = Badge::new("A11Y").variant(BadgeVariant::Default);
             a11y_badge.render(renderer, Rect { x: badge_x, y: badge_y, width: 45.0, height: 22.0 });
        }
    }
    ...
    fn render_detail_view(&self, renderer: &mut dyn Renderer, rect: Rect, system: &DesignSystem) {
        ...
        renderer.draw_text(&system.system.data.to_uppercase(), rect.x + 40.0, rect.y + 40.0, 32.0, [1.0, 1.0, 1.0, 1.0]);
        renderer.draw_text(&system.company.data, rect.x + 40.0, rect.y + 80.0, 18.0, [0.0, 0.8, 1.0, 1.0]);
        ...
        repo_btn.render(renderer, Rect { x: btn_x, y: btn_y, width: 140.0, height: 40.0 });
        btn_x += 160.0;
        
        let compare_btn = Button::new("Add to Comparison", || {});
        compare_btn.render(renderer, Rect { x: btn_x, y: btn_y, width: 160.0, height: 40.0 });
    }
    ...
    fn render_comparison_view(&self, renderer: &mut dyn Renderer, rect: Rect) {
        ...
        let systems_to_compare: Vec<&DesignSystem> = self.systems.iter()
            .filter(|s| self.selected_systems.contains(&s.system.data))
            .collect();
            
        if systems_to_compare.is_empty() {
            renderer.draw_text("No systems selected for comparison.", rect.x + 40.0, rect.y + 40.0, 16.0, [0.5, 0.5, 0.6, 1.0]);
            renderer.draw_text("Go back to catalog and add systems to compare them side-by-side.", rect.x + 40.0, rect.y + 70.0, 14.0, [0.4, 0.4, 0.5, 1.0]);
            return;
        }
        ...
        for system in &systems_to_compare {
            renderer.draw_text(&system.system.data.to_uppercase(), x + 10.0, rect.y + 40.0, 14.0, [0.0, 0.8, 1.0, 1.0]);
            x += col_w;
        }
        ...
        let rows: Vec<(&str, Box<dyn Fn(&DesignSystem) -> &str>)> = vec![
            ...
            ("A11Y", Box::new(|s: &DesignSystem| &s.accessibility_guidelines.data)),
        ];
        ...
        for (label, getter) in &rows {
            ...
            y += 40.0;
            renderer.draw_line(rect.x + 20.0, y - 15.0, rect.x + rect.width - 20.0, y - 15.0, [0.2, 0.2, 0.3, 0.3], 1.0);
        }
    }
}
