...
impl EirMotion {
    ...
    pub fn keyframe(mut self, anim_name: &str, time: f32, value: f32, easing: Easing) -> Self {
        if let Some(anim) = self.animations.iter_mut().find(|a| a.name == anim_name) {
            anim.keyframes.push(MotionKeyframe { time, value, easing });
        }
        self
    }
...
impl View for EirMotion {
    ...
    fn render(&self, renderer: &mut dyn Renderer, rect: Rect) {
        ...
        renderer.draw_text("Eir Motion Engine", rect.x + 10.0, rect.y + 20.0, 14.0, [0.7, 0.8, 1.0, 1.0]);

        // State indicator
        renderer.draw_text(
            &format!("State: {}", self.state_machine.current_state),
            rect.x + 120.0, rect.y + 20.0, 10.0, [0.5, 0.8, 1.0, 1.0]
        );
        ...
        for anim in &self.animations {
            renderer.fill_rect(
                Rect { x: rect.x + 10.0, y, width: rect.width - 20.0, height: 25.0 },
                [0.06, 0.06, 0.08, 1.0]
            );

            let _progress = format!("{:.0}%", (anim.keyframes.len() as f32 / (anim.duration / 0.1)) * 100.0);
            renderer.draw_text(&anim.name, rect.x + 15.0, y + 8.0, 11.0, [0.8, 0.9, 1.0, 1.0]);
            renderer.draw_text(&format!("{:.1}s", anim.duration), rect.x + rect.width - 60.0, y + 8.0, 9.0, [0.6, 0.7, 0.9, 1.0]);
            y += 28.0;
        }
        ...
        if !self.physics.is_empty() {
            ...
            renderer.draw_text("Physics Systems:", rect.x + 10.0, physics_y, 10.0, [0.7, 0.8, 1.0, 1.0]);

            for (i, p) in self.physics.iter().enumerate() {
                let bar_width = (p.velocity.abs() * 20.0).min(rect.width - 40.0);
                renderer.fill_rect(
                    Rect { x: rect.x + 15.0, y: physics_y + 15.0 + i as f32 * 15.0, width: bar_width, height: 8.0 },
                    [0.4, 0.6, 0.9, 1.0]
                );
            }
        }
...
impl LayoutView for EirMotion {
    fn size_that_fits(&self, _proposal: SizeProposal, _subviews: &[&dyn LayoutView], _cache: &mut LayoutCache) -> Size {
        Size { width: 300.0, height: 100.0 + self.animations.len() as f32 * 28.0 }
    }
