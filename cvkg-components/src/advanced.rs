...
impl<V> HuginChat<V> {
    ...
    pub fn message(mut self, is_self: bool, msg: V) -> Self {
        self.messages.push((is_self, msg));
        self
    }
}
impl<V> Default for HuginChat<V> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<V: View> View for HuginChat<V> {
    ...
    fn render(&self, renderer: &mut dyn Renderer, rect: Rect) {
        ...
        for (is_self, msg) in &self.messages {
            ...
            let bubble_x = if *is_self {
                rect.x + rect.width - bubble_w
            } else {
                rect.x
            };
