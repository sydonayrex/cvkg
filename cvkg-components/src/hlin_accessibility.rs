...
impl View for HlinAccessibility {
    ...
    fn render(&self, renderer: &mut dyn Renderer, rect: Rect) {
        let bg = if self.high_contrast { [0.0, 0.0, 0.0, 1.0] } else { [0.08, 0.08, 0.12, 1.0] };
        renderer.fill_rect(rect, bg);

        renderer.draw_text("Hlin Accessibility", rect.x + 10.0, rect.y + 20.0, 14.0, [0.6, 0.9, 0.6, 1.0]);

        // Status indicators
        let status_y = rect.y + 45.0;
        let hc_text = if self.high_contrast { "High Contrast: ON" } else { "High Contrast: OFF" };
        let rm_text = if self.reduced_motion { "Reduced Motion: ON" } else { "Reduced Motion: OFF" };

        renderer.draw_text(hc_text, rect.x + 10.0, status_y, 10.0, [0.7, 0.9, 0.7, 1.0]);
        renderer.draw_text(rm_text, rect.x + 130.0, status_y, 10.0, [0.7, 0.9, 0.7, 1.0]);

        // Focus indicator
        if let Some(focused) = &self.focus_state.focused_id {
            renderer.draw_text(&format!("Focused: {}", focused), rect.x + 10.0, status_y + 20.0, 10.0, [0.8, 0.9, 1.0, 1.0]);
        }
        ...
        for node in &self.tree {
            ...
            renderer.draw_text(&format!("{} {} - {}", role_icon, node.id, node.label), rect.x + 15.0, y, 9.0, [0.8, 0.9, 1.0, 1.0]);
            y += 18.0;
        }
    }
}
...
impl LayoutView for HlinAccessibility {
    fn size_that_fits(&self, _proposal: SizeProposal, _subviews: &[&dyn LayoutView], _cache: &mut LayoutCache) -> Size {
        Size { width: 280.0, height: 80.0 + self.tree.len() as f32 * 18.0 }
    }
