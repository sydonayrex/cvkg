...
impl IdunnPersistence {
    ...
    pub fn component_state(mut self, snapshot_id: &str, key: &str, state: &str) -> Self {
        if let Some(snap) = self.snapshots.iter_mut().find(|s| s.id == snapshot_id) {
            snap.component_states.insert(key.to_string(), state.to_string());
        }
        self
    }
    ...
    pub fn session(mut self, session_id: &str, workspace_id: &str) -> Self {
        self.active_session = Some(SessionRestore {
            ...
            restore_time: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs_f64(),
        });
        self
    }
...
impl View for IdunnPersistence {
    ...
    fn render(&self, renderer: &mut dyn Renderer, rect: Rect) {
        ...
        renderer.draw_text("Idunn Persistence", rect.x + 10.0, rect.y + 9.0, 13.0, [0.6, 0.9, 0.6, 1.0]);

        // Auto-restore indicator
        let auto_text = if self.auto_restore { "Auto-Restore: ON" } else { "Auto-Restore: OFF" };
        renderer.draw_text(auto_text, rect.x + 120.0, rect.y + 9.0, 10.0, [0.5, 0.8, 0.5, 1.0]);

        // Active session
        if let Some(session) = &self.active_session {
            renderer.draw_text(
                &format!("Session: {}", session.session_id),
                rect.x + 10.0, rect.y + 45.0, 11.0, [0.7, 0.9, 0.7, 1.0]
            );
            renderer.draw_text(
                &format!("Workspace: {}", session.workspace_id),
                rect.x + 10.0, rect.y + 62.0, 10.0, [0.5, 0.7, 0.5, 1.0]
            );
        }
        ...
        for snap in &self.snapshots {
            let age = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs_f64() - snap.timestamp;
            ...
            renderer.fill_rect(
                Rect { x: rect.x + 15.0, y, width: rect.width - 30.0, height: 22.0 },
                [0.05, 0.07, 0.05, 1.0]
            );
            renderer.draw_text(&snap.name, rect.x + 20.0, y + 6.0, 10.0, [0.7, 0.9, 0.7, 1.0]);
            renderer.draw_text(&age_str, rect.x + rect.width - 60.0, y + 6.0, 9.0, [0.5, 0.7, 0.5, 1.0]);
            y += 26.0;
        }
    }
}
...
impl LayoutView for IdunnPersistence {
    fn size_that_fits(&self, _proposal: SizeProposal, _subviews: &[&dyn LayoutView], _cache: &mut LayoutCache) -> Size {
        Size { width: 260.0, height: 100.0 + self.snapshots.len() as f32 * 26.0 }
    }
