...
fn tactical_hud_showcase() -> impl View {
    VStack::new(24.0)
        .child(
            ...
            RunesCard::new()
                .header(Text::new("Tactical Command Center").size(24.0).color([0.0, 1.0, 1.0, 1.0]))
                .content(
                    HStack::new(20.0)
                        .child(EikonaAvatar::new("Commander").status(AvatarStatus::Online))
                        .child(MerkiBadge::new("Sector 7G").color([1.0, 0.84, 0.0, 1.0]))
                )
        )
        .child(
        ...
        .child(
            // 3. Narrative & History
            SagaAccordion::new()
                .item("Mission Log: Ragnarok Init", UrdrTimeline::new()
                    .event("System Boot", "08:00")
                    .event("Niflheim Connection Established", "08:05")
                    .event("Anomaly Detected", "08:15")
                )
                .item("Resource Allocation", DraumaSkeleton::new())
        )
        .child(
            // 4. Interactive Tools
            HStack::new(20.0)
                .child(ValhallaRating::new(4.5).max(5))
                .child(BifrostColorPicker::new([0.0, 0.8, 1.0, 1.0]))
        )
}
